...
def mitigate_vqe_expectation(
    circuit: cirq.Circuit,
    hamiltonian: List[Tuple[float, cirq.PauliString]],
    techniques: List[str] = ['zne'],
    simulator: Optional[cirq.Simulator] = None,
    **kwargs
) -> float:
    """
    Mitigate expectation value for a VQE circuit.
    
    Args:
        circuit: Quantum circuit
        hamiltonian: List of (weight, PauliString)
        techniques: Mitigation techniques to apply
        simulator: Cirq simulator
        
    Returns:
        float: Mitigated expectation value
    """
    mitigator = ErrorMitigation(simulator)
    
    # Apply combined mitigation
    mitigated_energy, _ = mitigator.combined_mitigation(
        circuit,
